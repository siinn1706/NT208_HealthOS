...
- Để test chat WebSocket qua cùng frontend tunnel, đặt `NEXT_PUBLIC_WS_URL=wss://<tunnel-host>` rồi restart `npm run dev`; dev proxy của frontend sẽ chuyển tiếp WebSocket upgrade `/ws` và `/v1/**` sang `CORE_API_URL`. (`NEXT_PUBLIC_WS_URL` đổi tên từ `NEXT_PUBLIC_CORE_WS_URL` từ 2026-06.)
