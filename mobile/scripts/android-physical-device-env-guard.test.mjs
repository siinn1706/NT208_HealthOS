...
test('resolveExpoPublicUrlValues mirrors Expo development mode files and expansion', () => withTempProject((root) => {
  writeFileSync(join(root, '.env'), 'EXPO_PUBLIC_CORE_API_URL=http://192.168.1.10:8000\n');
  ...
  assert.deepEqual(values.map(({ key, value }) => ({ key, value })), [
    { key: 'EXPO_PUBLIC_CORE_API_URL', value: 'http://192.168.1.10:8000' },
...
test('findPhysicalDeviceBlockedUrls flags emulator and loopback URLs', () => {
  const blocked = findPhysicalDeviceBlockedUrls([
    { key: 'EXPO_PUBLIC_CORE_API_URL', value: 'http://10.0.2.2:8000', source: '.env' },
    { key: 'EXPO_PUBLIC_CORE_WS_URL', value: 'ws://192.168.1.10:8000', source: '.env' },
...
test('findPhysicalDeviceInvalidUrls rejects invalid schemes and wildcard hosts', () => {
  const invalid = findPhysicalDeviceInvalidUrls([
    { key: 'EXPO_PUBLIC_CORE_API_URL', value: 'not-a-url', source: '.env' },
    { key: 'EXPO_PUBLIC_CORE_WS_URL', value: 'http://192.168.1.10:8000', source: '.env' },
  ...
  assert.deepEqual(invalid.map(({ key, reason }) => ({ key, reason })), [
    { key: 'EXPO_PUBLIC_CORE_API_URL', reason: 'must be an absolute URL' },
    { key: 'EXPO_PUBLIC_CORE_WS_URL', reason: 'must use ws: or wss:' },
...
test('findPhysicalDeviceMissingUrls flags blank and absent required URLs', () => {
  const missing = findPhysicalDeviceMissingUrls([
    { key: 'EXPO_PUBLIC_CORE_API_URL', value: '', source: '.env' },
    { key: 'EXPO_PUBLIC_CORE_WS_URL', value: 'ws://192.168.1.10:8000', source: '.env' },
  ]);
...
test('assertPhysicalDeviceReachableEnv fails fast for physical phone plus Expo mode env', () => withTempProject((root) => {
  writeFileSync(join(root, '.env.development.local'), [
    'LOCAL_CORE_HOST=10.0.2.2',
    'EXPO_PUBLIC_CORE_WS_URL=ws://192.168.1.10:8000',
    'EXPO_PUBLIC_WEB_APP_URL=http://192.168.1.10:3000',
    'EXPO_PUBLIC_CORE_API_URL=http://${LOCAL_CORE_HOST}:8000',
  ].join('\n'));

  assert.throws(
    () => assertPhysicalDeviceReachableEnv({ device: physical, projectRoot: root, env: {} }),
    /Physical Android device R5CT12345 needs explicit LAN[\s\S]*EXPO_PUBLIC_CORE_API_URL=http:\/\/10\.0\.2\.2:8000/,
  );
}));

test('assertPhysicalDeviceReachableEnv fails fast for malformed physical phone config', () => withTempProject((root) => {
  writeFileSync(join(root, '.env'), [
    'EXPO_PUBLIC_CORE_API_URL=not-a-url',
    'EXPO_PUBLIC_CORE_WS_URL=http://192.168.1.10:8000',
    'EXPO_PUBLIC_WEB_APP_URL=ftp://192.168.1.10:3000',
  ].join('\n'));

  assert.throws(
    () => assertPhysicalDeviceReachableEnv({ device: physical, projectRoot: root, env: {} }),
    /EXPO_PUBLIC_CORE_API_URL=not-a-url[\s\S]*must be an absolute URL[\s\S]*EXPO_PUBLIC_CORE_WS_URL=http:\/\/192\.168\.1\.10:8000[\s\S]*must use ws: or wss:[\s\S]*EXPO_PUBLIC_WEB_APP_URL=ftp:\/\/192\.168\.1\.10:3000[\s\S]*must use http: or https:/,
  );
}));
...
test('assertPhysicalDeviceReachableEnv fails fast for blank physical phone config', () => withTempProject((root) => {
  ...
  assert.throws(
    () => assertPhysicalDeviceReachableEnv({ device: physical, projectRoot: root, env: {} }),
    /EXPO_PUBLIC_CORE_API_URL is blank or missing[\s\S]*Blank values can fall back to emulator-only 10\.0\.2\.2/,
  );
}));
...
test('assertPhysicalDeviceReachableEnv fails fast for missing physical phone config', () => withTempProject((root) => {
  ...
  assert.throws(
    () => assertPhysicalDeviceReachableEnv({ device: physical, projectRoot: root, env: {} }),
    /EXPO_PUBLIC_CORE_WS_URL is blank or missing[\s\S]*EXPO_PUBLIC_WEB_APP_URL is blank or missing/,
  );
}));
