...
export function EmergencyCard({ onPress, onShare, variant = 'gradient' }: EmergencyCardProps) {
        ...
        <View style={[
          ...
          <View style={styles.row}>
            ...
            <View style={styles.text}>
              <Text style={[typography.bodyMed, { color: t.ink }]}>Emergency card</Text>
  ...
  return (
    <Pressable onPress={onShare ?? onPress} style={({ pressed }) => pressed && { opacity: 0.85 }}>
      ...
      >
        <View style={styles.row}>
          <View style={[styles.iconTile, { backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 10 }]}>
            <IconHeartPulse size={20} color="#FFF" />
          </View>
          <View style={styles.text}>
            <Text style={[typography.bodyMed, { color: '#FFF' }]}>Emergency card</Text>
            <Text style={[typography.caption, { color: 'rgba(255,255,255,0.9)', marginTop: 1 }]}>Share vitals & allergies instantly</Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  softCard: { padding: 16, marginVertical: 8, borderWidth: 1 },
