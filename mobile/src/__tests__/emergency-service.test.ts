...
describe('emergencyService', () => {
  it('loads owner emergency profile', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith('/v1/users/me/emergency-profile');
  });
  ...
  it('creates token with acknowledge flag', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith('/v1/users/me/emergency-profile/tokens', {
      method: 'POST',
  ...
  it('revokes a single token', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith('/v1/users/me/emergency-profile/tokens/tok%2F1', {
      method: 'DELETE',
    });
  });
  ...
  it('revokes all active tokens', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith('/v1/users/me/emergency-profile/tokens/revoke-all', {
      method: 'POST',
    });
  });
  ...
  it('builds access-log query params', async () => {
    ...
    expect(mockApiRequest).toHaveBeenCalledWith('/v1/users/me/emergency-profile/access-logs?limit=5&token_id=tok-1');
  });
});
