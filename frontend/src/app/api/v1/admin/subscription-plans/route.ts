/**
 * BFF route: GET /api/v1/admin/subscription-plans
 *
 * Guards the request with an admin session check, then proxies to
 * Core's /v1/admin/subscription-plans. The coreProxy handles the 5-second
 * upstream timeout and maps transport errors to 502.
 *
 * Requirements: 3.6, 3.9, 3.10, 3.12
 */
...
export async function GET(request: NextRequest) {
  ...
  return coreProxy(request, "/v1/admin/subscription-plans");
}
