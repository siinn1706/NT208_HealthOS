...
describe('TimeRangeSelector', () => {
  it('renders 7d, 30d, 90d buttons', () => {
    render(<TimeRangeSelector value="7d" onChange={vi.fn()} />, { wrapper: IntlWrapper });
  ...
  it('calls onChange with correct period when button clicked', () => {
    ...
    render(<TimeRangeSelector value="7d" onChange={onChange} />, { wrapper: IntlWrapper });
    fireEvent.click(screen.getByText('30 ngày'));
    expect(onChange).toHaveBeenCalledWith('30d');
  });

  it('highlights active period button', () => {
    render(<TimeRangeSelector value="30d" onChange={vi.fn()} />, { wrapper: IntlWrapper });
