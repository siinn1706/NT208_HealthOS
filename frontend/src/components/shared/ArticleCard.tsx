...
export function ArticleCard({ article, variant = "list", categoryLabel = "" }: ArticleCardProps) {
    ...
    return (
      <article className="group flex gap-4 rounded-xl">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="mb-2 text-xs text-night-100/70 line-clamp-2">{excerpt}</p>
  ...
  if (variant === "rail") {
    return (
      <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-border/60 bg-card transition-[border-color,box-shadow,transform] duration-200 hover:-translate-y-0.5 hover:border-night-400/60 hover:shadow-md">
...
function MetaRow({ author, date, minRead, tone = "light", compact = false }: MetaRowProps) {
  const text =
    tone === "dark"
      ? compact
        ? "text-[10px] text-night-100/70"
        : "text-xs text-night-100/70"
      : compact
